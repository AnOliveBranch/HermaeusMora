        ...
        if (group === null) {
            ...
            if (subcommand === 'help') {
                ...
                checkAuthentication(user)
                    .then((result) => {
                        if (result === 'valid') {
                            ...
                            interaction.editReply({
                                content:
                                    "This shouldn't be able to happen. Contact @Robotic#1111 to investigate",
                                ephemeral: true
                            });
                            logErrorMessage(
                ...
                if (tokens.get(interaction.user.id) === undefined) {
                    ...
                    interaction.reply({
                        content:
                            'Your NexusMods API key is no longer being stored',
                        ephemeral: true
                    });
                    if (logVerbose === 'true') {
                        logInfoMessage(
...
async function logInfoMessage(message) {
    if (logInfo === 'true' && infoLogChannel !== null) {
        infoLogChannel.send(message).catch((err) => {
            logErrorMessage(
                `Caught error trying to info log ${message}\n${err}`
            );
        });
    }
}
    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(
            ...
            (res) => {
                res.on('error', (err) => {
                    logErrorMessage(
                        `Error getting mod files for ${ganeName} mod ${modId}: ${err}`
                    );
                ...
                res.on('end', () => {
                    ...
                    if (files.hasOwnProperty('code')) {
                        ...
                        if (errorCode === 404) {
                            ...
                            console.log(files);
                            reject(
                                'An unknown error occured retrieving this mod'
                            );
                        }
                        return;
    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(
            ...
            (res) => {
                res.on('error', (err) => {
                    logErrorMessage(
                        `Error getting mod info for ${ganeName} mod ${modId}: ${err}`
                    );
...
function getVersions(filesJSON, info) {
    ...
    let versionString = `Found the following versions for ${
        info.name
    }: ${versions.join(', ')}`;
